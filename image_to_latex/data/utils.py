...
class Tokenizer:
    ...
    def load(cls, filename: Union[Path, str]) -> "Tokenizer":
        ...
        Tokenizer.vocab_dict=token_to_index
...
def get_all_formulas(filename: Path) -> List[List[str]]:
    ...
    with open(filename) as f:
        all_formulas = [FMM_func(Tokenizer.vocab_dict.keys(),formula) for formula in f.readlines()]
#    for i in all_formulas:
#        Tokenizer.max_length=len(i) if len(i)>Tokenizer.max_length else Tokenizer.max_length
    return all_formulas
